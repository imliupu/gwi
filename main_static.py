...
def train_class_worker(args):
    ...
    for epoch in range(args.epochs):
        ...
        t_acc = accuracy_score(train_targets_list, train_preds_list)
        t_prec = precision_score(train_targets_list, train_preds_list, zero_division=0)
        t_rec = recall_score(train_targets_list, train_preds_list, zero_division=0)
        t_f1 = f1_score(train_targets_list, train_preds_list, zero_division=0)
        
        print("\n" + "#"*60)
        print(f" >>> Epoch {epoch} Summary")
        print("#"*60)
        print_confusion_matrix(train_targets_list, train_preds_list, title=f"TRAIN Set (PID {pid} Ep {epoch})")
        logging.info(f"[TRAIN] Ep {epoch}: Loss={avg_train_loss:.4f} | Acc={t_acc:.4f}, Prec={t_prec:.4f}, Rec={t_rec:.4f}, F1={t_f1:.4f}")
        ...
        v_acc = accuracy_score(val_targets_list, val_preds_list)
        v_prec = precision_score(val_targets_list, val_preds_list, zero_division=0)
        v_rec = recall_score(val_targets_list, val_preds_list, zero_division=0)
        v_f1 = f1_score(val_targets_list, val_preds_list, zero_division=0)
        
        val_cm = print_confusion_matrix(val_targets_list, val_preds_list, title=f"VALIDATION Set (PID {pid} Ep {epoch})")
        logging.info(f"[VAL]   Ep {epoch}: Loss={avg_val_loss:.4f}   | Acc={v_acc:.4f}, Prec={v_prec:.4f}, Rec={v_rec:.4f}, F1={v_f1:.4f}")

        if v_f1 > best_f1 and HAS_PLOT:
            try:
                plt.figure(figsize=(6, 5))
                sns.heatmap(val_cm, annot=True, fmt='d', cmap='Blues', xticklabels=['Normal', 'Infiltration'], yticklabels=['Normal', 'Infiltration'])
                plt.title(f'VAL CM (PID {pid}, Ep {epoch}, F1={v_f1:.3f})')
        ...
        if v_f1 > best_f1:
            ...
            logging.info(f" >>> New Best Model Saved! Val F1={v_f1:.4f}")
        else:
            patience_count += 1
...
def run_dispatcher(args):
    ...
    for pid in range(args.n_partitions):
        ...
        cmd = [sys.executable, 'main_static.py', 
               '--worker_mode', 
               '--pid', str(pid), 
